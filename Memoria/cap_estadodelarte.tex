...
mecanismos \cite[p. 77]{vacca2013}:
\begin{itemize}
...
Teniendo esta idea clara, es preciso constatar que este proyecto se centra, sobre todo, en la creación de pseudoanónimos y en el estudio de técnicas para disgregar la identidad física
...
Una característica de estos túneles es que son de sentido único. Por este motivo, para que una instancia I2P pueda intervenir en las dos partes de la comunicación (emisión y recepción de paquetes) debe crear túneles de salida y de entrada. Los datos que viajan entre cada enrutador del túnel van cifrados y cada enrutador que recibe el paquete sólo puede acceder a la información correspondiente al siguiente \textit{router} al que se le debe enviar el paquete.
	...
	\begin{figure}[h]
		\centerline{
			\mbox{\includegraphics[width=4.00in]{images/tunnel_i2p.png}}
		}
...
No obstante, y debido a que este trabajo está orientado al desarrollo de una herramienta \textbf{Python}, a continuación se explicará de manera breve cómo es posible crear un servicio oculto haciendo uso de varias bibliotecas Python, entre ellas \textbf{Stem y Flask}.
...
A continuación, generaremos una contraseña que utilizaremos para iniciar un controlador de la biblioteca Stem (ver Figura~\ref{fig:TorPwd}):
