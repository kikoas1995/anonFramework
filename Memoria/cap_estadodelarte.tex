...
ambiguos como la filosofía. \comments{Aquí faltan referencias. No olvide el texto de Danezis al que hago referencia en el capítulo 1: \cite{danezis2010critical}}
...
\end{figure}
\comments{Las figuras deben citadas y referencias en el texto mediante
  \ref{fig:norm_Daugman}. Veo que has empleado la misma etiqueta para
  todas las figuras: cada figura debe tener una etiqueta distinta. }

La privacidad no debe confundirse con el \textbf{anonimato en la red}
\comments{Aquí conviene que hables algo de identidad digital. Una
  misma identidad física puede tener asociadas múltiples identidades
  digitales, cada una de las cuales se emplea para hacer un uso de un
  servicio en el ciberespacio. La proyección de la identidad física en
  el ciberespacio es llevada típicamente a cabo por algún proveedor de
  servicio, el cual extrae información del usuario a través de las
  tres vías indicadas en el capítulo 1 de
  \cite{lane2014privacy}. Además, dicha proyección se realizar de
  diversas formas: mediante autenticación de usuarios (e.g., usuario y
  contraseña), pero a veces también es posible identificar usuarios a
  través de direcciones IP o mediante el análsis de la información que
  un usuario envía al proveedor de servicio. Dime si no te queda claro
  este punto. Es muy importante que el texto recoja esta matriz de
  complejidades en la gestión de la identidad digital.}. Éste último
...
frecuentemente en este proyecto, y es el de \textbf{ofuscación}. \comments{Faltan referencias. Puedes utilizar el libro que te recomendé \cite{brunton2015obfuscation}}
...
\comments{Faltan referencias, por ejemplo: \cite{pfitzmann2010terminology}}
...
\comments{OJO, esta sección parece un poco desconectada del resto. Lo que interesa reseñar aquí son problemas relativos a la correcta gestión de la identidad digital y al tratamietno inadecuado de datos
  privados de usuarios. Te pongo algunos ejemplos:}
\begin{itemize}
  ...
  Conviene empezar el capítulo recalcando que Internet no fue
  ...
  \commented{en riesgo (por diversos motivos) la privacidad}
  \comments{hasta ahora has hablado de seguridad. De hecho, lo que te
    interesa es destacar problemas de privacidad.} del usuario en la
  red.
...
En el año 1983, se hizo mandatorio que los usuarios de la red ARPANET
...
En los últimos años la variedad de ataques de robo de información ha
...
\label{sec:a_favor}
\comments{No es del todo correcto que hables aquí de protección de
  identidad. Es cierto que se protege la identidad frente a cualquier
  intruso en el canal de comunicación, pero no ocurre lo mismo con
  respecto al destino de las comunicaciones. En este caso, además de
  ofuscar el origen de las comunicaciones, por ejemplo, hemos de
  emplear un perfil falso o una identida anónima, además de evitar
  enviar información al proveedor que permita nuestra
  re-identificación. }
...
\comments{Es cierto que estás haciendo ofuscación, pero sólo de
  tráfico de red. Conviene que subrayes esto y lo diferencies de otras
...
\comments{Falta referencias}
\subsection{Servidor proxy}

Un servidor proxy es básicamente un mediador entre un usuario que
...
\commented{Éste} \comments{revisa esto en todo el texto: cuando no es pronombre no lleva tilde, OJO!!!} tipo de proxy es muy comúnmente utilizado para proteger la
...
Tras este experimento, en el año siguiente Xu Wei continuó
...
Veamos una comparativa entre un paquete IP original, uno que hace uso
del modo transporte y otro del modo túnel de IPsec:
 ...
 \subsubsection{OpenVPN}
	...
	El caso de los VPN, hasta el momento es imposible interceptar
        ...
        expuestos.  \comments{Conviene reseñar los diversos servicios
          VPN existentes, algunos de ellos dados a modo de plugin de
          Firefox, Chrome (Tunnelo, Hoxx, etc.) Aquí sería pertinente
          que uses el listado de herramientas que figure en la página
          que te envíe: no todos los servicios VPN son iguales en
          términos de protección del anonimato de red. }
\subsection {Deep-web}
...
Para empezar, la \textit{deep-web} se refiere a aquellos contenidos
que no están indexados por los prinicipales motores de búsqueda, tales
como Google o Bing \comments{falta una referencia}. Por ende, es bastante complicado localizarlos, ni
...
Otro término muy utilizado es el de \commented{\textit{darknet}}
\comments{Esto es algo que debe ir en el glosario.}. Una
...
En numerosas ocasiones se relaciona dicho tipo de redes con la
...
\begin{figure}[h]
	...
	\label{fig:norm_Daugman}
\end{figure}
\comments{Pon la url desde la que has obtenido la figura. Además, pon la fecha del último acceso. }

Sin embargo, el objetivo de estas herramientas no fue el motivar los actos ilícitos. 

El uso que le de den la mayoría de usuarios no es el que originalmente
...
Éste modelo descentralizado, con el que se almacena en el disco duro
...
\comments{No sólo aplica en el campo del lenguaje natural, sino que
  también hay que tenerlo en cuenta para cualquier tipo de lenguaje
  \cite{caliskan2015coding,caliskan2015anonymizing}. } La estilometría
...
gobierno federal. \comments{falta referencia}
