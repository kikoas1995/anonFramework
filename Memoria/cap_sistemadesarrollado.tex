...
La herramienta podrá ser ejecutada sin que el usuario final necesite instalar bibliotecas extra (más allá del servicio Tor).\\
\linebreak
...
Se ha elegido Python (en su versión 2.7) puesto que la gran mayoría de herramientas de auditoría han sido diseñadas en este lenguaje, y es posible reutilizar código así como importar bibliotecas muy útiles en el campo de la seguridad informática.
...
En todo el proyecto se ha seguido un proceso de \textit{web-scraping}~\cite{article:webscraping} (utilizando distintas bibliotecas) para poder utilizar los servicios de un sitio web de forma \textbf{sencilla, por consola, rápida y automática en la gran parte de los casos.}
...
En la \textbf{primera} se hace uso de la página \url{https://tempail.com/} para poder visualizar los correos entrantes en tiempo real directamente desde la interfaz del programa (ver Figura~\ref{fig:tempail_inbox}). Se hizo uso de las bibliotecas \textbf{requests} y \textbf{html} para lograr la funcionalidad deseada. A continuación una captura de la bandeja de entrada en funcionamiento:
...
La otra bandeja de entrada volátil hace uso de la web \url{https://temptami.com/}, un servicio de bandeja de entrada temporal algo menos conocido que tempail y, por ende, con menos restricciones para el uso en registros de diversas páginas como FaceBook. A diferencia de los anteriores módulos, esta página hace necesario realizar \textit{web-scraping} teniendo en cuenta código JavaScript. Es por ello que la biblioteca para examinar la página ahora es \textbf{Selenium}, una biblioteca que provee al usuario de automatización de procesos en páginas web, mediante un \textbf{WebDriver}, que es el nombre de la interfaz donde se ejecutan dichos procesos. 
...
Todos estos módulos han sido implementados usando la biblioteca \textbf{Selenium}, debido a la existencia de código JavaScript en prácticamente todas las páginas estudiadas. 
...
El encriptación, la cual usa un esquema AES de 256 bit transparente al usuario, se consigue mediante el uso de la biblioteca \textbf{pysqlcipher}.
...
La herramienta desarrollada hace uso de las siguientes bibliotecas:
...
Además de estas, se ha realizado un estudio de la red Tor, así como sus servicios ocultos, donde se han utilizado las siguientes bibliotecas:
\begin{itemize}
